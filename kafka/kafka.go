/*
 * This is local copy of testcontainers/testcontainers/go/modules/kafka/kafka.go with modified startup script for
 * the purpose of fully working demonstration. See https://github.com/testcontainers/testcontainers-go/issues/2630
 */
package kafka
...
func WithListener(listener string) Option {
	return func(e *KafkaContainer) error {
...
func copyStarterScript(ctx context.Context, c testcontainers.Container) error {
	...
	advertisedListener := fmt.Sprintf("PLAINTEXT://%s:%d,BROKER://%s:9092", host, port.Int(), hostname)
	advertisedListenerFromEnv := findAdvertisedListener(inspect.Config.Env)
	if len(advertisedListenerFromEnv) > 1 {
		advertisedListener = advertisedListenerFromEnv
	}

	customizedStartupScriptContent := fmt.Sprintf(starterScriptContent, advertisedListener)
	fmt.Println(customizedStartupScriptContent)
	scriptContent := fmt.Sprintf(customizedStartupScriptContent, host, port.Int(), hostname)
...
func findAdvertisedListener(environments []string) string {
	for i := range environments {
		if strings.HasPrefix(environments[i], "KAFKA_ADVERTISED_LISTENERS=") {
			return strings.Split(environments[i], "=")[1]
		}
	}
	return ""
}
